...
def jsonMatchNormalization(res) :
    # [(match_id, date, status, league_id, league_string, home_id, home_string, home_logo, home_score, away_id, away_string, away_logo, away_score, isDeployed, contractAddress)]
    matchesData = []
    ...
    for i in range(res['resultSet']['count']) :
        ...
        # date to timestamp
        match_timestamp = datetime.timestamp(datetime.strptime(date, '%Y-%m-%dT%H:%M:%SZ')) - config_bet["offSetApiBug"]*60*60
        
        matchesData.append((match_id, match_timestamp, status, league_id, league_string, home_id, home_string, home_logo, home_score, away_id, away_string, away_logo, away_score, isDeployed, address))
    return matchesData
